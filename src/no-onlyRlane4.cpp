...
public:
	...
	Mat draw_lane(Mat frame, Mat warped_frame, Mat left_coef, Mat right_coef, Mat Minv) {
		...
		Point left_point[height];
		Point right_point[height];		

		Point warped_left_point[height];
		Point warped_right_point[height];

		vector<Point> left_points;
		vector<Point> right_points;
		
		for(int i=0;i<height;i++) {
			left_point[i].x = left_coef.at<float>(2, 0) * pow(i, 2) + left_coef.at<float>(1, 0) * i + left_coef.at<float>(0, 0);
			left_point[i].y = i;
			right_point[i].x = right_coef.at<float>(2, 0) * pow(i, 2) + right_coef.at<float>(1, 0) * i + right_coef.at<float>(0, 0);
			right_point[i].y = i;
		}
		...
		for(int i=0;i<height;i++) {
			...
			left_points.push_back(warped_left_point[i]);
			right_points.push_back(warped_right_point[i]);
		}
