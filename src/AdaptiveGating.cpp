...
{
	ROS_INFO("Start");
    sub = n.subscribe("/filtered_msgs",1000, &AdaptiveGating::Callback, this);
...
//	ROS_INFO("-----------------start");
	...
	int msg_size = 0;
	for(int i=0; i<100; i++){ // Warning!! Please check msg size
//		cout << msg.can_msgs[i].id << ' ';
		if((msg.can_msgs[i].id >= 0x500) && (msg.can_msgs[i].id <= 0x540)){msg_size++;}
		else if((msg.can_msgs[i].id >= 0x739) && (msg.can_msgs[i].id <= 0x756)){msg_size++;}
...
//		printf("msg id : %d %d\n",msg.can_msgs[i].id,i);
		...
		int int_value = 0;
		if(msg.can_msgs[i].id == 0x540){
			printf("Radar ID : 0x%02X [%d %d %d %d %d %d %d %d]\n", msg.can_msgs[i].id,msg.can_msgs[i].data[0],msg.can_msgs[i].data[1],msg.can_msgs[i].data[2],msg.can_msgs[i].data[3],msg.can_msgs[i].data[4],msg.can_msgs[i].data[5],msg.can_msgs[i].data[6],msg.can_msgs[i].data[7]);

			for(int j=0; j<8; j++){ // A msg has 8 bits and has Multiplexor Signal
				vec[vec_count].push_back(msg.can_msgs[i].data[j]);
			}
			
			cout << "vec[" << vec_count << "] :";
			for(int j=0; j<8; j++){
				cout <<" "<< vec[vec_count][j] ;
			}
		...
		if((msg.can_msgs[i].id >= 0x500) && (msg.can_msgs[i].id <= 0x53F))
		{
			// Delphi Radar POS X,Y
			int r_index = 0;
			int mov_flag;
			int r_sign = (msg.can_msgs[i].data[1] & 0x10)>>4;
			float radar_range;
			float radar_angle;
			
			r_index = msg.can_msgs[i].id - 0x500;

			radar_range = (float)(((msg.can_msgs[i].data[2] & 0x07)<<8) + msg.can_msgs[i].data[3])*(0.1);
			if(r_sign == 0){
				radar_angle = (float)(((msg.can_msgs[i].data[1] & 0x1F)<<5) + ((msg.can_msgs[i].data[2] & 0xF8)>>3))*(0.1);
			}
			if(r_sign == 1){
				radar_angle = (float)(1024 - (((msg.can_msgs[i].data[1] & 0x1F)<<5) + ((msg.can_msgs[i].data[2] & 0xF8)>>3)))*(-0.1);
			}

//			printf("Radar ID : 0x%02X  %0.1fm / %0.1fdeg\n", msg.can_msgs[i].id, radar_range, radar_angle);

			if((radar_range > 1) && (radar_range < 150)){
//				printf("Radar ID : 0x%02X  %0.1fm / %0.1fdeg\n", msg.can_msgs[i].id, radar_range, radar_angle);
				radar_pos_x = radar_range*cos(radar_angle*M_PI/180);
				radar_pos_y = (-1)*radar_range*sin(radar_angle*M_PI/180);
//				printf("Radar ID : 0x%02X  x : %0.1f  y : %0.1f\n", msg.can_msgs[i].id, radar_pos_x, radar_pos_y);
...
//		}
		...
		else if((msg.can_msgs[i].id >= 0x739) && (msg.can_msgs[i].id <= 0x756))
		{
			if((msg.can_msgs[i].id % 3)==1){
    			int c_index = 0;
    			c_index = msg.can_msgs[i].id - 1849;
    			int c_sign = (msg.can_msgs[i].data[3] & 0x80)>>7;
    			float c_width = (float)((msg.can_msgs[i].data[6]>>2) + ((msg.can_msgs[i].data[7] & 0x01)<<6))*0.05;
    
    			camera_pos_x = (float)(((msg.can_msgs[i].data[2] & 0x0F)<<8) + msg.can_msgs[i].data[1])*0.05;
    
    			if(c_sign == 0){
    				camera_pos_y = (float)((msg.can_msgs[i].data[3]<<4) + ((msg.can_msgs[i].data[2]&0xF0)>>4))*(-0.05);
    			}
    			else if(c_sign == 1){ 
    				camera_pos_y = (float)(4096 - ((msg.can_msgs[i].data[3]<<4) + ((msg.can_msgs[i].data[2]&0xF0)>>4)))*(0.05);
    			}
...
//    			printf("Mobileye ID : 0x%02X  x:%0.1f y:%0.1f\n", msg.can_msgs[i].id, camera_pos_x, camera_pos_y);
			}
		}
