...
//	ROS_INFO("-----------------start");
	...
	for(int i=0; i<70; i++){
		...
		else if((msg.can_msgs[i].id >= 1280) && (msg.can_msgs[i].id <= 1343 )){msg_size++;} // Delphi ESR MMR
...
//	cout << "sizeof msg : " << sizeof(msg) << endl;
	for(int i=0;i<msg_size;i++){
		...
		if((msg.can_msgs[i].id >= 257) && (msg.can_msgs[i].id <= 272))
		{
			...
			radar_pos_x = (float)((((msg.can_msgs[i].data[2] & 0x7F)<<8) + msg.can_msgs[i].data[1])*0.01);
			radar_pos_y = (float)((((msg.can_msgs[i].data[4] & 0x3F)<<8) + msg.can_msgs[i].data[3])*(-0.01) + 81.91);
...
//    			cout << "p_sign :"<<p_sign<<" R_lane_pos :" << R_lane_position << endl;
		...
		else if((msg.can_msgs[i].id >= 1280) && (msg.can_msgs[i].id <= 1343))
		{
			int esr_index = 0;
			float esr_angle = 0;
			float esr_range = 0;
			int esr_sign = (msg.can_msgs[i].data[1] & 0x10)>>4;

			esr_index = msg.can_msgs[i].id - 1280;

			if(esr_sign == 0){
				esr_angle = (float)(((msg.can_msgs[i].data[2] & 0xF8)>>3) + ((msg.can_msgs[i].data[1] & 0x1F)<<5))*0.1;
			}
			else if(esr_sign == 1){
				esr_angle = (float)(1024 - (((msg.can_msgs[i].data[2] & 0xF8)>>3) + ((msg.can_msgs[i].data[1] & 0x1F)<<5)))*(-0.1);
			}
			
			esr_range = (float)((msg.can_msgs[i].data[3] & 0xFF) + ((msg.can_msgs[i].data[2] & 0x07)<<8))*0.1;

			cout << "esr_index : " << esr_index << " esr_angle : " << esr_angle << " esr_range : " << esr_range << endl; 

			esr_pos_x = esr_range*cos(esr_angle*M_PI/180);
			esr_pos_y = esr_range*sin(esr_angle*M_PI/180);

			cout << "esr_x : "<< esr_pos_x << " esr_y : "<< esr_pos_y << endl;

			esr_x[esr_index] = esr_pos_x;
			esr_y[esr_index] = esr_pos_y;
			esrx_float.push_back(esr_pos_x);
			esry_float.push_back(esr_pos_y);

//			if((radar_pos_x < 150) && (radar_pos_y > -10) && (radar_pos_y < 10)){
//    			if((mov_flag == 1) || (mov_flag == 3))
//    			{
//    				radar_x[esr_index] = radar_pos_x;
//    				radar_y[esr_index] = radar_pos_y;
//    
//    			}
//			}
		}
...
//	cout << "ffusion------" << endl;
	...
	for(int i=0; i<esrx_float.size(); i++){
		pt.x = esrx_float[i];
		pt.y = esry_float[i];
		mk_esr.points.push_back(pt);
//		cout << cx_float[i] << endl;
	}	
//	cout << "camera------" << endl;
...
//-------------------------------------------------radar
	mk_esr.header.frame_id = "mk";
	mk_esr.type = visualization_msgs::Marker::SPHERE_LIST;
    mk_esr.header.stamp = ros::Time::now();
	mk_esr.lifetime = ros::Duration();
	mk_esr.ns = "mk_esr";
	mk_esr.id = 9;
