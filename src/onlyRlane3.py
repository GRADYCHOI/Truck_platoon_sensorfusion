...
from platooning_lane_det.msg import Range 
from platooning_lane_det.msg import lane 
from platooning_lane_det.msg import multican 
...
def hls_lthresh(img, thresh=(240, 255)): #200
...
def sliding_window_polyfit(img, last_leftx_base, last_rightx_base, last_left_fit, check):
    ...
    rightx_base = np.argmax(histogram[midpoint+quarter_point+left_offset:(midpoint+quarter_point + 3*left_offset)]) + midpoint+quarter_point+left_offset
    ...
    def cancallback(self,data):
        ...
        for i in range(len(data.can_msgs)):
            ...
            if(target_id == data.can_msgs[i].id):
                distance = (((data2 & 0x7F)<<8) + data1)*0.01/2
